...
func (s *scraper) scrape(_ context.Context) (pmetric.Metrics, error) {
	...
	if err != nil {
		...
		if !strings.Contains(strings.ToLower(err.Error()), "locked") {
			...
			// locked and unavailable. We do not want to log an error message
			// on every poll for this case.
			// See: https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/18236
			s.settings.Logger.Debug("failed collecting partitions information: %w", zap.Error(err))
		} else {
